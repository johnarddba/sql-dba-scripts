...
FROM
    sys.databases
WHERE
    name NOT IN ('master', 'tempdb', 'model', 'msdb', 'hsadmin')
...
WHILE EXISTS (
    ...
    FROM
        ...
        BEGIN
    SELECT
        TOP 1 @CurrentDB = DBName
    FROM
        #DBList
    SET
        @F0 = @BackupFolder + '\' + @Servername + ' \ ' + @CurrentDB + ' \ ' + ' wk_ '+ @wkYear + ' _ ' + DATENAME(dw, GETDATE()) + ' _ '+@o + ' _ ' + @CurrentDB  + ' _ ' + ' DR_Prod_FULL ' + ' _part0.LBK ';
	...
	BEGIN TRY
		...
		SET @SQL = N' ALTER DATABASE ' + ' [' + @CurrentDB + '] ' + N'
    SET
